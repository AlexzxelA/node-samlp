...
module.exports.auth = function(options) {
  ...
  return function (req, res, next) {
    ...
    if(!opts.idpInitiatedSessionHandler && req.method === 'GET' && req.query.Signature){
    ...
    function execute (postUrl, audience, req, res, next) {
      ...
      if (!user) return opts.idpInitiatedSessionHandler? opts.idpInitiatedSessionHandler(401, null):res.send(401);

      opts.audience = audience;
      opts.postUrl = postUrl;

      getSamlResponse(opts, user, function (err, SAMLResponse) {
        if (err) return opts.idpInitiatedSessionHandler?opts.idpInitiatedSessionHandler(err, null):next(err);
        ...
        else if(opts.idpInitiatedSessionHandler){
            return opts.idpInitiatedSessionHandler(null, templates.form({
	            type:            'SAMLResponse',
	            callback:        postUrl,
    ...
    if(opts.idpInitiatedSessionHandler)
